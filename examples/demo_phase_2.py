...
async def demo_phase_2_workflow():
    ...
    try:
        ...
        result = await server.app.call_tool("create_midi_file", {
            "title": "Demo Song",
            "tempo": 128,
            "time_signature": [4, 4],
            "key_signature": "G"
        ...
        for track in tracks:
            result = await server.app.call_tool("add_track", {
        ...
        print("\n3. Saving MIDI file...")
        result = await server.app.call_tool("save_midi_file", {
            "midi_file_id": file_id,
            "filename": "demo_song.mid"
        })
        print(f"   Result: {result[0][0].text}")
        
        print("\n4. Analyzing the file...")
        result = await server.app.call_tool("analyze_midi_file", {
            "midi_file_id": file_id
        })
        print("   Analysis:")
