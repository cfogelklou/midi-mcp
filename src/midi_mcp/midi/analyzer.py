...
class MidiAnalyzer:
    ...
    def __init__(self):
        ...
        self.gm_instruments = {
            ...
            50: "SynthStrings 1", 51: "SynthStrings 2", 52: "Choir Aahs",
            ...
            107: "Koto", 108: "Kalimba", 109: "Bag pipe", 110: "Fiddle",
    ...
    def _analyze_instruments(self, midi_file) -> Dict[str, Any]:
        ...
        for channel, programs in programs_per_channel.items():
            instruments_by_channel[channel] = [
                {
                    'program': program,
                    'instrument': self.gm_instruments.get(program, f"Unknown ({program})")
                }
        ...
        return {
            ...
            'instrument_list': [
                self.gm_instruments.get(program, f"Unknown ({program})")
                for program in sorted(unique_instruments)
            ]
        }
    
    def _analyze_channels(self, midi_file) -> Dict[str, Any]:
        """Analyze MIDI channel usage."""
        channel_usage = defaultdict(lambda: {
            'note_events': 0,
            'control_events': 0,
            'program_changes': 0,
            'notes': [],
