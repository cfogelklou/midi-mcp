    ...
    def get_scale_degrees(self, scale: Scale, note_names: bool = True) -> Dict[int, any]:
        ...
        for i, note in enumerate(scale.notes, 1):
            degree_info = {
                ...
                'degree_name': self._get_degree_name(i)
            }
            if note_names:
    ...
    def find_relative_scales(self, scale: Scale) -> Dict[str, Scale]:
        ...
        if scale.name == 'major':
            ...
            if len(scale.notes) >= 6:
                ...
                relatives['relative_minor'] = self.generate_scale(
                    minor_root.name, 'natural_minor', minor_root.octave
                )
            ...
            relatives['relative_major'] = self.generate_scale(
                major_root.name, 'major', major_root.octave
            )
        
        # Parallel minor/major
        if scale.name == 'major':
            relatives['parallel_minor'] = self.generate_scale(
                scale.root.name, 'natural_minor', scale.root.octave
            )
        elif scale.name in ['natural_minor', 'aeolian']:
            relatives['parallel_major'] = self.generate_scale(
                scale.root.name, 'major', scale.root.octave
            )
