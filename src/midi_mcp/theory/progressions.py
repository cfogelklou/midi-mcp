...
class ProgressionManager:
    ...
    def create_chord_progression(
        self, key: str, progression: List[str], duration_per_chord: float = 1.0, voicing: str = "close"
    ) -> ChordProgression:
        """
        ...
        for roman_numeral in progression:
            ...
            try:
                chord = self.chord_manager.build_chord(
                    root_note=chord_root, chord_type=chord_type, voicing=voicing, octave=4
                )
                chords.append(chord)
                ...
                chord = self.chord_manager.build_chord(
                    root_note=chord_root, chord_type="major", voicing=voicing, octave=4
                )
                chords.append(chord)
                durations.append(duration_per_chord)

        return ChordProgression(chords=chords, key=key, roman_numerals=progression, durations=durations)
    ...
    def analyze_progression(self, chord_symbols: List[str], key: str = None) -> Dict[str, any]:
        ...
        for i, symbol in enumerate(chord_symbols):
            ...
            # Look for cadences
            if i > 0:
                cadence = self._identify_cadence(analysis["roman_numerals"][i - 1], roman, is_minor)
                if cadence:
                    analysis["cadences"].append(
                        {"position": i, "type": cadence, "chords": [chord_symbols[i - 1], symbol]}
                    )

            # Check for secondary dominants
            if self._is_secondary_dominant(roman, is_minor):
                analysis["secondary_dominants"].append(
                    {"position": i, "chord": symbol, "roman": roman, "target": self._get_secondary_target(roman)}
                )

        return analysis

    def suggest_next_chord(
        self, current_progression: List[str], key: str, style: str = "common_practice"
    ) -> Dict[str, any]:
        """
        ...
        if last_chord in tendencies:
            for next_chord, probability in tendencies[last_chord].items():
                ...
                suggestions.append({"chord": next_chord, "probability": probability, "reason": reason})
    ...
    def transpose_progression(self, progression: ChordProgression, target_key: str) -> ChordProgression:
        """
    ...
    def _is_problematic_progression(self, from_chord: str, to_chord: str) -> bool:
        ...
        problematic = [("I", "ii"), ("iii", "IV"), ("vi", "vii")]  # Weak progression  # Uncommon  # Weak
    ...
    def _has_parallel_motion_risk(self, from_chord: str, to_chord: str) -> bool:
        ...
        risk_progressions = [("I", "V"), ("IV", "V")]  # Risk if not voice-led properly  # Common risk area
