...
class KeyManager:
    ...
    def detect_key(self, midi_notes: List[int], timestamps: Optional[List[float]] = None) -> KeyAnalysis:
        ...
        """
        if not midi_notes:
            return KeyAnalysis(most_likely_key="C", confidence=0.0, alternative_keys=[], key_changes=[])
        ...
        return KeyAnalysis(
            most_likely_key=best_key, confidence=confidence, alternative_keys=alternatives, key_changes=key_changes
        )

    def analyze_modulations(self, midi_notes: List[int], timestamps: List[float]) -> List[Dict[str, any]]:
        """
    ...
    def _detect_key_changes(self, midi_notes: List[int], timestamps: List[float]) -> List[Tuple[float, str, float]]:
        ...
        for i in range(0, len(midi_notes) - window_size, step_size):
            ...
            if current_key is None or (analysis.most_likely_key != current_key and analysis.confidence > 0.75):

                changes.append((window_time, analysis.most_likely_key, analysis.confidence))
