...
from ..models.theory_models import (
    HarmonicAnalysis, KeyAnalysis, VoiceLeadingAnalysis, 
    ChordProgression, Note, Chord
)
...
class MusicAnalyzer:
    ...
    def analyze_midi_file(self, midi_notes: List[int], 
                         timestamps: Optional[List[float]] = None,
                         track_info: Optional[Dict] = None) -> HarmonicAnalysis:
        """
        ...
        chord_progression = self._analyze_harmonic_progression(
            midi_notes, timestamps, key_analysis.most_likely_key
        )
        
        # Step 3: Voice Leading Analysis
        voice_leading = self.voice_leading_manager.validate_voice_leading(
        ...
        non_chord_tones = self._analyze_non_chord_tones(
            midi_notes, timestamps, chord_progression
        )
        
        # Step 7: Harmonic Rhythm Analysis
        harmonic_rhythm = self._analyze_harmonic_rhythm(
            chord_progression, timestamps
        )
    ...
    def analyze_chord_sequence(self, chord_symbols: List[str],
        ...
        """
        # Analyze progression
        progression_analysis = self.progression_manager.analyze_progression(
            chord_symbols, key
        )
    ...
    def suggest_harmonic_improvements(self, chord_progression: ChordProgression) -> List[str]:
        ...
        vl_analysis = self.voice_leading_manager.validate_voice_leading(
            chord_progression.chords
        )
        ...
        bass_movement = sum(abs(bass_notes[i+1] - bass_notes[i]) 
    ...
    def identify_musical_form(self, chord_progression: ChordProgression,
                            section_markers: Optional[List[float]] = None) -> Dict[str, any]:
        """
    ...
    def _analyze_harmonic_progression(self, midi_notes: List[int],
                                   timestamps: Optional[List[float]],
                                   key: str) -> ChordProgression:
        ...
        if not timestamps:
            ...
            if chord_notes:
                ...
                chord = self.chord_manager.build_chord(
                    root_note=best_match['root'],
                    chord_type=best_match['chord_type']
                )
                return ChordProgression(
                    chords=[chord],
                    key=key,
                    roman_numerals=['I'],  # Simplified
                    durations=[1.0]
                )
    ...
    def _analyze_non_chord_tones(self, midi_notes: List[int],
                               timestamps: Optional[List[float]],
                               chord_progression: ChordProgression) -> List[Dict[str, any]]:
        ...
        for i, (note, time) in enumerate(zip(midi_notes, timestamps)):
            ...
            if chord_at_time:
                ...
                if note_pc not in chord_pcs:
                    ...
                    nct_type = self._classify_non_chord_tone(
                        note, i, midi_notes, timestamps, chord_at_time
                    )
                    ...
                    non_chord_tones.append({
                        ...
                        'time': time,
                        'type': nct_type,
                        'chord_context': chord_at_time.symbol
                    })
        
        return non_chord_tones
    
    def _analyze_harmonic_rhythm(self, chord_progression: ChordProgression,
                               timestamps: Optional[List[float]]) -> List[float]:
    ...
    def _group_notes_by_time(self, midi_notes: List[int],
                           timestamps: List[float]) -> List[Tuple[List[int], float]]:
    ...
    def _find_chord_at_time(self, time: float, 
                          chord_progression: ChordProgression) -> Optional[Chord]:
    ...
    def _classify_non_chord_tone(self, note: int, position: int,
                               melody: List[int], timestamps: List[float],
                               chord: Chord) -> str:
    ...
    def _analyze_phrase_structure(self, chord_progression: ChordProgression) -> List[int]:
        ...
        for i, roman in enumerate(chord_progression.roman_numerals):
            ...
            if (i < len(chord_progression.roman_numerals) - 1 and
                roman == 'V' and chord_progression.roman_numerals[i + 1] == 'I'):
    ...
    def _guess_musical_form(self, total_length: int, patterns: List[Dict],
                          phrase_lengths: List[int]) -> str:
