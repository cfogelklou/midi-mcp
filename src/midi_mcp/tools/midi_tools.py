...
def register_midi_tools(app: FastMCP, registry: ToolRegistry, midi_manager: MidiManager) -> None:
    """
    ...
    discover_tool = Tool(
        ...
        description="Discover all available MIDI devices on the system",
        inputSchema={
            "type": "object",
            "properties": {},
            "required": []
    ...
    connect_tool = Tool(
        ...
        inputSchema={
            ...
            "properties": {
                "device_id": {
                    "type": "string",
                    "description": "ID of the device to connect to"
                }
            },
            "required": ["device_id"]
        }
    ...
    play_note_tool = Tool(
        ...
        inputSchema={
            ...
            "properties": {
                "device_id": {
                    "type": "string",
                    "description": "ID of the device to play the note on"
                },
                "note": {
                    "type": ["integer", "string"],
    ...
    async def play_midi_note(
        ...
        try:
            ...
            if not 0 <= note_number <= 127:
                return [TextContent(
                    type="text",
                    text=f"Invalid note number: {note_number}. Must be 0-127."
                )]
            
            if not 0.1 <= duration <= 10.0:
                return [TextContent(
                    type="text",
                    text=f"Invalid duration: {duration}. Must be 0.1-10.0 seconds."
                )]
            ...
            return [TextContent(
                type="text",
                text=f"Played {note_name} for {duration} seconds on {device.device_info.name}"
            )]
    ...
    async def list_connected_devices() -> List[TextContent]:
        ...
        try:
            ...
            if not connected_devices:
                return [TextContent(
                    type="text",
                    text="No MIDI devices are currently connected."
                )]
    ...
    disconnect_tool = Tool(
        ...
        inputSchema={
            ...
            "properties": {
                "device_id": {
                    "type": "string",
                    "description": "ID of the device to disconnect from"
                }
            },
            "required": ["device_id"]
        }
