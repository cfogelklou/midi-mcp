...
from ..models.composition_models import (
    CompleteComposition, CompositionAnalysis, SongStructure, Section, 
    Melody, Arrangement, TextureLevel, DynamicLevel
)
...
class CompleteComposer:
    ...
    def compose_complete_song(
        ...
        try:
            ...
            request = self._parse_composition_request(
            ...
            arrangement = self._create_full_arrangement(
                request, song_structure, main_melody, base_progression
            )
            
            # Step 6: Apply texture orchestration
            logger.info("Orchestrating texture changes")
            orchestrated_composition = self._apply_texture_orchestration(
            ...
            complete_composition = CompleteComposition(
                ...
                melody={
                    ...
                    "rhythm": main_melody.rhythm,
                    "register": main_melody.register
                },
    ...
    def _create_harmonic_foundation(
        ...
        progression_result = self.composer.create_progression(
            request.genre, request.key, "standard"
        )
    ...
    def _develop_melodic_materials(
        self,
        main_melody: Melody,
        song_structure: SongStructure
    ) -> Dict[str, Melody]:
        ...
        for i, section in enumerate(song_structure.sections):
            ...
            if section_type == "verse":
                ...
                variation = self.melody_variator.create_variation(
                    main_melody.notes, "rhythmic"
                )
                ...
                development = self.motif_developer.develop_motif(
                    motif_obj, ["sequence", "inversion"], 8
                )
    ...
    def _create_full_arrangement(
        ...
        composition = {
            ...
            "harmony": harmony,
            "structure": {
                "sections": [{"type": section.type.value} for section in song_structure.sections]
            }
        }
        ...
        arrangement = self.ensemble_arranger.arrange_for_ensemble(
            composition, request.ensemble_type, arrangement_style
        )
        
        return arrangement
    
    def _apply_texture_orchestration(
        self,
        arrangement: Arrangement,
        song_structure: SongStructure
    ) -> Dict[str, Any]:
        ...
        orchestrated = self.texture_orchestrator.orchestrate_texture_changes(
            composition_dict, dynamic_plan
        )
    ...
    def _create_detailed_sections(
        self,
        song_structure: SongStructure,
        melodic_variations: Dict[str, Melody]
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_composition_notes(
        self,
        request: CompositionRequest,
        orchestrated_composition: Dict[str, Any]
    ) -> List[str]:
...
class CompositionAnalyzer:
    ...
    def analyze_composition_quality(self, composition: CompleteComposition) -> CompositionAnalysis:
        ...
        try:
            ...
            overall_score = self._calculate_overall_score(
                melody_analysis, harmonic_analysis, rhythmic_analysis, 
                structural_analysis, arrangement_analysis
            )
    ...
    def _analyze_melody_quality(self, melody: Melody) -> Dict[str, Any]:
        ...
        direction_balance = min(ascending, descending) / max(ascending, descending) if max(ascending, descending) > 0 else 0
...
class CompositionRefiner:
    ...
    def _refine_melody(self, composition: CompleteComposition) -> Tuple[CompleteComposition, List[str]]:
        ...
        if composition.main_melody and composition.main_melody.notes:
            ...
            variation = self.melody_variator.create_variation(
                composition.main_melody.notes, "embellishment"
            )
    ...
    def _refine_harmony(self, composition: CompleteComposition) -> Tuple[CompleteComposition, List[str]]:
        ...
        if composition.harmonic_progression:
            ...
            enhanced = self.chromatic_harmony_generator.add_chromatic_harmony(
                chord_symbols, composition.key, "medium"
            )
