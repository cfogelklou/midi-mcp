...
class VoiceLeadingOptimizer:
    ...
    def optimize_voice_leading(
        ...
        if not chord_progression[0].get("voicing"):
            first_voicing = self._create_initial_voicing(
                chord_progression[0]["chord"], voice_count
            )
    ...
    def _optimize_single_voicing(
        ...
        for _ in range(10):  # Try 10 different voicings
            voicing = self._generate_candidate_voicing(
                chord_tones, previous_voicing, voice_count
            )
...
class ChromaticHarmonyGenerator:
    ...
    def _analyze_harmonic_content(self, progression: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "chromaticism_percentage": (functions.get("secondary_dominant", 0) + 
                                     functions.get("applied_chord", 0) + 
                                     functions.get("modal_mixture", 0)) / len(progression)
        }
...
class BassLineCreator:
    ...
    def _get_chord_root(self, chord_symbol: str) -> int:
        ...
        roots = {
            "C": 0, "Cm": 0, "C7": 0, "Cmaj7": 0,
            "F": 5, "Fm": 5, "F7": 5, "Fmaj7": 5,
            "G": 7, "Gm": 7, "G7": 7, "Gmaj7": 7,
            "Am": 9, "A": 9, "A7": 9, "Amaj7": 9,
            "Dm": 2, "D": 2, "D7": 2, "Dmaj7": 2,
            "Em": 4, "E": 4, "E7": 4, "Emaj7": 4,
            "Bm": 11, "B": 11, "B7": 11, "Bmaj7": 11
        }
    ...
    def _analyze_harmonic_support(self, bass_notes: List[int], chord_progression: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "harmonic_clarity": "high" if root_support_percentage > 0.7 else "medium" if root_support_percentage > 0.4 else "low"
        }