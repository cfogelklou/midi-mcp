...
class TestMusicAnalyzer:
    ...
    def test_analyze_midi_file_chord_progression(self, music_analyzer):
        ...
        midi_notes = ([60, 64, 67] * 4 +      # C major
                     [57, 60, 64] * 4 +      # A minor  
                     [53, 57, 60] * 4 +      # F major
                     [55, 59, 62] * 4)       # G major
        
        # 4 beats per chord
        timestamps = ([i * 0.25 for i in range(12)] +
    ...
    def test_suggest_harmonic_improvements_weak_progression(self, music_analyzer):
        ...
        chord_progression = progression_manager.create_chord_progression(
            key="C",
            progression=["I", "ii", "iii", "vi"],  # No strong resolution
            duration_per_chord=1.0
        )
    ...
    def test_identify_musical_form_longer_piece(self, music_analyzer):
        ...
        progression = (["I", "V", "vi", "IV"] * 4 +  # 16 bars
                      ["ii", "V", "I", "vi"] * 2)    # 8 bars
    ...
    def test_harmonic_rhythm_analysis(self, music_analyzer):
        ...
        timestamps = [0.0, 0.1, 0.2, 2.0, 2.1, 2.2, 3.0, 3.1, 3.2,  # Slow-fast-medium
                     5.0, 5.1, 5.2, 5.5, 5.6, 5.7, 6.0, 6.1, 6.2,   # Medium rhythm
                     7.0, 7.1, 7.2, 7.25, 7.3, 7.35]                  # Fast rhythm
